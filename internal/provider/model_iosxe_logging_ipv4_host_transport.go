...
func (data LoggingIPv4HostTransport) getPath() string {
	return fmt.Sprintf("Cisco-IOS-XE-native:native/logging/host/ipv4-host-transport-list=%s", url.QueryEscape(fmt.Sprintf("%v", data.Ipv4Host.Value)))
}

// if last path element has a key -> remove it
func (data LoggingIPv4HostTransport) getPathShort() string {
	path := data.getPath()
...
func (data LoggingIPv4HostTransport) toBody(ctx context.Context) string {
	body := `{"` + helpers.LastElement(data.getPath()) + `":{}}`
	if !data.Ipv4Host.Null && !data.Ipv4Host.Unknown {
		body, _ = sjson.Set(body, helpers.LastElement(data.getPath())+"."+"ipv4-host", data.Ipv4Host.Value)
	}
...
func (data *LoggingIPv4HostTransport) updateFromBody(ctx context.Context, res gjson.Result) {
	prefix := helpers.LastElement(data.getPath()) + "."
	if res.Get(helpers.LastElement(data.getPath())).IsArray() {
		prefix += "0."
	}
	if value := res.Get(prefix + "ipv4-host"); value.Exists() {
		data.Ipv4Host.Value = value.String()
	} else {
		data.Ipv4Host.Null = true
	}
...
func (data *LoggingIPv4HostTransport) fromBody(ctx context.Context, res gjson.Result) {
	prefix := helpers.LastElement(data.getPath()) + "."
	if res.Get(helpers.LastElement(data.getPath())).IsArray() {
		prefix += "0."
	}
	if value := res.Get(prefix + "transport.udp.port-config"); value.Exists() {
		data.TransportUdpPorts = make([]LoggingIPv4HostTransportTransportUdpPorts, 0)
		value.ForEach(func(k, v gjson.Result) bool {
			item := LoggingIPv4HostTransportTransportUdpPorts{}
	...
	if value := res.Get(prefix + "transport.tcp.port-config"); value.Exists() {
		data.TransportTcpPorts = make([]LoggingIPv4HostTransportTransportTcpPorts, 0)
		value.ForEach(func(k, v gjson.Result) bool {
			item := LoggingIPv4HostTransportTransportTcpPorts{}
			if cValue := v.Get("port-number"); cValue.Exists() {
				item.PortNumber.Value = cValue.Int()
				item.PortNumber.Null = false
			}
	...
	if value := res.Get(prefix + "transport.tls.port"); value.Exists() {
		data.TransportTlsPorts = make([]LoggingIPv4HostTransportTransportTlsPorts, 0)
		value.ForEach(func(k, v gjson.Result) bool {
			item := LoggingIPv4HostTransportTransportTlsPorts{}
			if cValue := v.Get("port-number"); cValue.Exists() {
				item.PortNumber.Value = cValue.Int()
				item.PortNumber.Null = false
			}
...
func (data *LoggingIPv4HostTransport) setUnknownValues(ctx context.Context) {
	...
	if data.Ipv4Host.Unknown {
		data.Ipv4Host.Unknown = false
		data.Ipv4Host.Null = true
	}
	for i := range data.TransportUdpPorts {
...
func (data *LoggingIPv4HostTransport) getDeletedListItems(ctx context.Context, state LoggingIPv4HostTransport) []string {
...
func (data *LoggingIPv4HostTransport) getEmptyLeafsDelete(ctx context.Context) []string {
