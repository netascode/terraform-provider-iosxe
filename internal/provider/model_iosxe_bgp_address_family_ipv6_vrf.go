...
func (data *BGPAddressFamilyIPv6VRF) updateFromBody(res gjson.Result) {
	...
	for i := range data.Vrfs {
		...
		if value := res.Get(fmt.Sprintf("%vvrf.#(name==\"%v\").ipv6-unicast.advertise.l2vpn.evpn", prefix, key)); value.Exists() {
			data.Vrfs[i].AdvertiseL2vpnEvpn.Value = true
		} else {
			data.Vrfs[i].AdvertiseL2vpnEvpn.Value = false
		}
		if value := res.Get(fmt.Sprintf("%vvrf.#(name==\"%v\").ipv6-unicast.redistribute-v6.connected", prefix, key)); value.Exists() {
			data.Vrfs[i].RedistributeConnected.Value = true
		} else {
			data.Vrfs[i].RedistributeConnected.Value = false
		}
		if value := res.Get(fmt.Sprintf("%vvrf.#(name==\"%v\").ipv6-unicast.redistribute-v6.static", prefix, key)); value.Exists() {
